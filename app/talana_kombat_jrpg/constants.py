from app.talana_kombat_jrpg.moves import Attacks, Moves
...
ARNOLD_TALADOKEN_COMBINATION = (
    f"{Moves.A.value}-{Moves.S.value}-{Moves.A.value}-{Attacks.P.value}"
)
...
TONY_TALADOKEN_COMBINATION = (
    f"{Moves.D.value}-{Moves.S.value}-{Moves.D.value}-{Attacks.P.value}"
)

...
from app.talana_kombat_jrpg.fighters.specials import Taladoken, Remuyuken
from app.talana_kombat_jrpg.moves import Attacks
from app.talana_kombat_jrpg.serializers import Game


class GenerateMoves:

    @classmethod
    def special(cls, name_fighter: str, moves_message: str, combination: str, energy: int, special_name: str) -> dict:
        special = None
        ...
        if prev_move[0]:
            ...
            final = f"{name_fighter} go to the {move} attack with a {special.name.upper()} "
        else:
    ...
    @classmethod
    def basic_moves(cls, name_fighter, moves_message, ):
